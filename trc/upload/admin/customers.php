...
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-confirm/3.3.0/jquery-confirm.min.js"></script>
      ...
      <div class="panel-body table-responsive">
        <div >
          <form class="fieldset-form" id="creat_customer">
            ...
            <input type="hidden" id="edit_id" name="edit_id" value="<?php if(isset($_POST["editid"])){ echo $_POST["editid"]; } ?>" >
  ...
  $(document).ready(function(){
    ...
    main_user['customer_id']= "<?php echo $_POST['editid'];?>";
    console.log(main_user);
    $.ajax({
       type: 'POST',
       url: '<?php echo $backend_ip; ?>index.php/customer_edit',
       data: main_user,
       dataType: 'json',
       beforeSend: function(){
       ...
       success: function (res){
           if(res){
             ...
             if(res['success'] == 1){
              //  alert('login');
               console.log(res.data[0]["customer_name"]);
               $('#customer_name').val(res.data[0]["customer_name"]);
               $('#customer_email').val(res.data[0]["customer_email"]);
               $('#customer_phone').val(res.data[0]["customer_phone"]);
               $('#customer_add').val(res.data[0]["customer_add"]);
               $('#customer_PanCard').val(res.data[0]["customer_pan"]);
               $('#customer_gst').val(res.data[0]["customer_gst"]);
